...
date_default_timezone_set('America/Lima');
	...
	if ( empty($_POST['txtid']) || empty($_POST['txtnom']) || empty($_POST['txtes']) || empty($_POST['txtra']) || empty($_POST['txtse']) || empty($_POST['txtpe']) || empty($_POST['txtfn']) || empty($_POST['txtco'])
	 ...
	 !empty($_POST['txtid']) && !empty($_POST['txtnom']) && !empty($_POST['txtes']) && !empty($_POST['txtra']) && !empty($_POST['txtse']) && !empty($_POST['txtpe']) && !empty($_POST['txtfn']) && !empty($_POST['txtco']) 
		 ...
		 if(empty($_FILES['txtfoto']['name'])){
                     $sql = "INSERT INTO mascota (nombre, especie,raza,sexo,pelaje,f_nac,activo,f_creacion,codigodvi,id_cliente)
       VALUES('".$nom."','".$es."','".$ra."','".$se."','".$pe."','".$fn."','".$estado."','".$Fecharegistro."','".$dvi."','".$idc."');";
	...
	$limite_kb = 16384;
       ...
       $sql = "INSERT INTO mascota (nombre, especie,raza,sexo,pelaje,f_nac,foto,activo,f_creacion,codigodvi,id_cliente)
       VALUES('".$nom."','".$es."','".$ra."','".$se."','".$pe."','".$fn."','".$fotob64."','".$estado."','".$Fecharegistro."','".$dvi."','".$idc."');";

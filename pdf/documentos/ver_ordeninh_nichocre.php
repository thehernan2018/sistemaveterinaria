...
*/
if (isset($_GET['idventacre'])){ 
$sql_factura=mysqli_query($con,"select toi.nroorden, concat(tps.nombre, ' ', tps.apellido) as solicitante, tps.direccion, tps.dni,td.ubicacion, concat(tpd.nombre, ' ', tpd.apellido) as difunto, toi.fallecio, toi.enfermedad, toi.papeletadifuncion, toi.municipalidad,toi.idventa, tcp.nrocompromiso ,toi.observaciones, toi.sepelio, toi.fecharegistro 
from t_ordeninhumacion toi, t_persona tpd, t_persona tps, t_difunto td, t_compromisopago tcp 
WHERE  
toi.idsolicitante =tps.idpersona  and 
toi.iddifunto  =tpd.idpersona and 
toi.iddifunto  =td.idpersona  and 
toi.idcompromisopago  =tcp.idcompromiso  and 
toi.idcompromisopago='".$_GET['idventacre']."'
order by toi.idventa desc 
");
	...
	$rw_factura=mysqli_fetch_row($sql_factura);
						...
						$idventa =$rw_factura[10]; 
						$idcompromisopago =$rw_factura[11]; 
						$observacionesd=$rw_factura[12]; 
						$sepeliod=$rw_factura[13]; 
						$fecharegistroo=$rw_factura[14];   
	$simbolo_moneda=get_row('perfil','moneda', 'id_perfil', 1);
	
	require_once(dirname(__FILE__).'/../html2pdf.class.php');
    // get the HTML
     ob_start();
     include(dirname('__FILE__').'/res/ver_ordeninh_nichocre_html.php');
